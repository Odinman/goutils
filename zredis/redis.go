...
func InitZRedis(servers interface{}, sentinels []string, passwd, db, mtag string) (zr *ZRedis, e error) {

	var realServers []string
	if ss, ok := servers.(string); ok {
		realServers = []string{ss}
	} else if ss, ok := servers.([]string); ok {
		realServers = ss
	} else {
		return nil, errors.New("input servers error")
	}
